<?php 
			...
			$cons = "SELECT * FROM reserva where nro_reserva = ".$Nreserva."";
			$rs = mysql_query($cons);
			while($col = mysql_fetch_array($rs)){

			$precio = $col['precio'];
			$pago = new cargaPago($precio,$fecha_actual,$Nreserva);
			$pago -> pago();
			}

	/******CLASE QUE GENEREA EL PDF******/		
			...
			$pdf -> Cell(0,5,'LINEAS AEREAS PLUMA - COMPROBANTE DE PAGO',0,1,'C');
			...
			$sql = "SELECT p.nombre as nombre ,p.apellido as apellido ,a1.nombre as aero_origen,a2.nombre as aero_destino,v.fecha_ida as ida ,v.fecha_vuelta as vuelta ,lv.hora as hora,r.precio as precio,pa.nro_pago as pago,pa.fecha as fecha,a1.ciudad ciudad1,a2.ciudad ciudad2
			...
			$pdf->Output('comprobantePago.pdf','D');
